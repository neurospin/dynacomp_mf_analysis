...
def single_mf_analysis(args):
    ...
    if True:
        ...
        for params_index in params_index_list: # iterate through mf parameters
            ...
            # store results
            subject_output_dir = os.path.join(output_dirs[group][subject], 'EOG')
            if not os.path.exists(subject_output_dir):
                os.makedirs(subject_output_dir)

            output_filename = os.path.join(subject_output_dir, condition + "_channel_EOG_params_%d"%(params_index) +'.h5')
...
if __name__ == '__main__':
    ...
    for args in arg_instances:
        ...
        output_filename = os.path.join(subject_output_dir, condition + "_channel_EOG_params_%d"%(params_index_list[-1]) +'.h5')
